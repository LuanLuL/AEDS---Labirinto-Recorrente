...
void Inventario::solveMaze(){
    int row = 0, column = 0;
    int currentRow = row, currentColumn = column;
    No* no = this->lista.getInicio();
    Matriz matriz = retrive("dataset/" + no->getValor());
    ...
    while(finishSolveMaze(no, row, column)){
        switch(matriz.randomStreet(&row, &column, &currentRow, &currentColumn)){
        	case 1:{
                ...
                create("dataset/" + no->getValor(), matriz);
                no->setVisit(true);
                no = no->getAnterior();
                matriz = retrive("dataset/" + no->getValor());
...
bool Inventario::finishSolveMaze(No *curretNo, int keyRow, int keyColumn){
    if(this->lista.allVisit() && (keyRow == 0 && keyColumn == 0) && (curretNo == this->lista.getInicio())){
        return false;
    }
    return true;
}
